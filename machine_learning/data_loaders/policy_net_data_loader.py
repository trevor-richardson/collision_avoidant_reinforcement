...
class VideoDataGenerator(object):
    def __init__(self, train_dir, val_dir, test_dir, train, val, test):
        ...
        #count training and testing data -- booya
    ...
    def prepare_data(self):
        ...
        hit_counter = 0
        miss_counter = 0
        sizeofval = val_number_of_hit + val_number_of_miss
        for enumerator in range(sizeofval):
            rand = random.uniform(0,1)
            if (miss_counter > val_number_of_hit -1):
                self.val.append(val_hit[hit_counter])
                self.val_class.append(classification1)
                hit_counter+=1
            elif (hit_counter > val_number_of_miss -1):
        ...
        hit_counter = 0
        miss_counter = 0
        sizeoftest = test_number_of_hit + test_number_of_miss
        for enumerator in range(sizeoftest):
            rand = random.uniform(0,1)
            if (miss_counter > test_number_of_miss -1):
                self.test.append(test_hit[hit_counter])
                self.test_class.append(classification1)
                hit_counter+=1
            elif (hit_counter > test_number_of_hit -1):
